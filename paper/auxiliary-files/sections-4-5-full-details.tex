...
Ref.~\cite{rubboli2026complete}. It uses the same classification, notation,
and witness families as the main paper. The explicit cross-references below
identify the corresponding main-paper statements. It
...
and give the principal proof ideas. The present paper supplies the corresponding
complete analytic proofs in the same classification, notation, and witness
families. Its endpoint-aware statements directly imply the main-paper
formulations.
...
In this section, we prove that the sufficient conditions obtained in Section~\ref{sec:sufficient-conditions} are necessary. We make no regularity assumption near $\alpha=1$. In particular, the two one-sided integrals of $|\alpha/(1-\alpha)|$ are not assumed finite; whenever such an integral occurs in the final parameter region, its finiteness is derived from monotonicity.
...
The next propositions and remarks give the temperate-necessity arguments.
Propositions~5.1--5.3 and~5.6 correspond to Propositions~V.1--V.4 of the main
paper. Remarks~5.4--5.5 state the homogeneity/channel lift and the compensated
witness used in these proofs.
...
$1-\int\omega\,\mathrm d\mu$, because one or both one-sided integrals may be infinite. We instead work with truncated coefficients, so the construction is well-defined without a global regularity assumption.
...
corresponds to Proposition~V.5 of the main paper. Proposition~5.8 gives the
positive-tropical necessity argument used in Step~4 of Proposition~III.7 of the
main paper.
