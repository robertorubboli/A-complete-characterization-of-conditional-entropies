...
with their proofs. Lean calls each named item a \emph{declaration}. A
declaration is simply a named entry that Lean stores, such as a definition,
lemma, or theorem. A theorem declaration records exactly which assumptions may
be used and which conclusion must be proved. When the project is built, Lean
checks every step using
Mathlib, its standard mathematical library, and declarations that have already
been proved in the project. This project rejects unfinished-proof markers, and
Lean accepts each remaining theorem only when its proof checks.
...
theorem's name finds its complete formal statement and the proof that combines
any required supporting lemmas. For example, Lemma~4.1 is represented by
\LeanDecl{ConditionalEntropy.fullDetailsLemma4_1}. A theorem may use several
smaller proved lemmas internally, just as a written proof may refer to earlier
results.
...
monotonicity for every admissible candidate, with each admissibility condition
written explicitly. It assumes no additional fact beyond those stated
conditions. Neither theorem uses a project-defined axiom. In the complete-proof
paper and in Lean, $\log$ is the natural logarithm and an independent fair bit
has value $\log 2$.
...
\node[final] (laws) at (2.8,0)
  {\texttt{allConditionalEntropy}\\\texttt{Axioms}\\five conditional-entropy laws};
...
152-row blueprint lists labeled definitions and results used to construct the
formalization, including supporting and final results. These rows describe the
internal structure and do not correspond to additional numbered manuscript
statements. The detailed dependency graph shows how the entries are connected.
...
no project-defined \texttt{axiom}. It also uses no project-defined
\texttt{opaque} declaration. Thus none of the manuscript results has been
inserted into the project as an unproved fact. The final theorems require no
extra project hypothesis. Like other Lean developments based on Mathlib, the
project uses Lean's standard logical foundations, including classical choice,
...
foundations used by the final theorems, and checks the 152 labeled definitions
and results recorded in the blueprint. It also reads all thirty-one result
numbers and labels from the LaTeX paper, verifies that their Lean names are
distinct and appear in the same order, and asks Lean to check every
corresponding theorem. A successful run therefore checks both the proofs and
the paper-to-Lean correspondence. The formalization covers the classification
and the five entropy laws for all four concrete candidate families, using the
natural-log convention of the complete-proof paper.
