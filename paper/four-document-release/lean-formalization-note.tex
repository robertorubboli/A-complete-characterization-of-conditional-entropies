...
boundary. The presentation follows the simple theorem-facing pattern of the
Lean correspondence appendix in Ref.~\cite{tomamichel2026weaklogmajorization}.
\end{abstract}
...
point from a conclusion to ingredients used in its proof, matching the
convention of Ref.~\cite{tomamichel2026weaklogmajorization}. Pale green boxes
...
\node[paper] (suff) at (-4.1,-1.8)
  {Sufficiency\\Props.~1.4--1.8};
\node[paper] (necess) at (0,-1.8)
  {Necessity\\Props.~2.1--2.7};
...
\node[infra] (stationary) at (3.4,-5.4)
  {Exact tropical\\stationarity and\\finite witnesses};
...
\end{tikzpicture}
\caption{Dependency graph for the Lean formalization. Arrows point to proof
ingredients. Color records proof role, not confidence: green means public
conclusion, blue means internally proved infrastructure, white means a
paper-facing result, and dashed gray marks the explicitly excluded upstream
representation theorem.}
\label{fig:lean-dependencies}
\end{figure}

Table~\ref{tab:lean-correspondence} follows the numbered statements of the
concise companion. All Lean names are in the namespace
\LeanDecl{ConditionalEntropy}. A split API means that the manuscript statement
is intentionally proved by several smaller declarations. The last row is
worded differently because Proposition~2.8 combines the formalized parameter
restriction with the upstream representation theorem just identified.

\renewcommand{\arraystretch}{1.12}
\begin{table}[htbp]
\centering
\small
\caption{Statement-by-statement correspondence.}
...
\centering
\small
\textbf{Table~\ref{tab:lean-correspondence} (continued).}\par\smallskip
\begin{tabular}{p{0.25\textwidth}p{0.49\textwidth}p{0.18\textwidth}}
\toprule
Manuscript statement & Lean declaration & Status\\
\midrule
...
\Formalized\ (split API)\\
\addlinespace
Proposition~2.2: Shannon-atom obstruction &
\LeanDecl{negativeShannonObstruction} & \Formalized\\
\addlinespace
Proposition~2.3: exceptional moment bound &
...
\addlinespace
Proposition~2.6: one exceptional upper point &
\LeanDecl{oneUpperPoint};
\LeanDecl{negativeTemperateNecessity_of_atom_zero} &
\Formalized\ (split API)\\
\addlinespace
Proposition~2.7: derivation necessity &
\LeanDecl{derivationNecessity} & \Formalized\\
\addlinespace
Proposition~2.8: represented derivations &
