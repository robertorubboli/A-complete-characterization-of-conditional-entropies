...
\begin{proof}[Proof idea]
The three exclusions use three separate localizers, exactly as in the Lean
proof.  An atom $m=\mu(\{1\})>0$ is detected by the rounded Shannon
perturbation with $p=q=1/2$, $R=c+2$, and scalar direction $(1,0)$: its
limiting logarithmic derivatives are $m$ and $0$, so the outer exponential has
limiting curvature $m^2>0$.

For the remaining two tests, choose a $\mu$-null threshold $r>0$, put
$R=r+1$, and use the two-block path having $\lceil d^R\rceil$ entries
$1+u_0\lambda$ and $\lceil d^{R-r}\rceil$ entries
$d(1+u_1\lambda)$.  If $r>1$ and upper support is present, the localized
coefficient
\[
A_r=\int_{(r,+\infty]}\frac{\alpha}{1-\alpha}\,\d\mu(\alpha)
\]
is negative.  The velocity $(u_0,u_1)=(0,1)$ gives limiting first and second
logarithmic derivatives $A_r$ and $-A_r$, hence curvature
$A_r^2-A_r>0$.  Thus no upper support is possible.  If $r<1$, write
\[
B_r=\int_{[0,r)}\frac{\alpha}{1-\alpha}\,\d\mu(\alpha).
\]
The velocity $(u_0,u_1)=(1,0)$ gives limiting derivatives $B_r$ and
$-B_r$, so concavity implies $B_r(B_r-1)\leq0$ and therefore $B_r\leq1$.
Letting null thresholds increase to $1$ and using monotone convergence proves
the stated moment bound.  These finite Shannon and two-block families are the
counterexamples formalized in Lean; endpoint-separated truncation justifies
their limits without a preliminary finiteness hypothesis.
\end{proof}
...
$m:=\mu(\{1\})<0$ and $A>0$ is a localized upper contribution, use the same
rounded Shannon family with $p=q=1/2$, $R=c+2$, and direction $(-A/m,1)$.
...
\begin{proof}[Proof idea]
Use the formalized three-block path with $R=a+b+1$, respective counts
$\lceil d^R\rceil$, $\lceil d^{R-a}\rceil$, and
$\lceil d^{R-a-b}\rceil$, and base values $1,d,d^2$.  If
Eq.~\eqref{eq:negative-truncated-moment-necessary} failed, then
$C:=1-A_b-B_a>0$.  The velocity
\[
...
$A_1,A_2>0$.  The three-block velocity
$\left(0,1/A_1,-1/A_2\right)$ cancels the limiting first derivative, while the
...
present.  With the signed measure $-\tau$, its stationary direction is
$(-A/m,1)$, where $m<0$ is the signed Shannon atom and $A>0$ is a localized
upper coefficient; the limiting second derivative is $-A<0$.

If two separated upper-support portions remain, their coefficients
$A_1,A_2>0$ and the three-block velocity
$\left(0,1/A_1,-1/A_2\right)$ give first derivative $0$ and second derivative
$-1/A_1-1/A_2<0$.  If
Eq.~\eqref{eq:negative-tropical-necessary-moment} fails in the exceptional
branch, the corresponding truncated coefficients $A,C>0$ and velocity
$\left(0,1/C,-1/A\right)$ instead give $-1/C-1/A<0$.
At finite dimension the first derivative is corrected to zero by a vanishing
transverse adjustment (respectively in directions $(0,1)$, $(0,1,0)$, and
$(0,0,1)$), after which quasiconvexity is contradicted.  Support isolation makes
the upper moment finite; monotone passage through lower truncations then proves
lower-moment finiteness.  These are precisely the Shannon, two-upper-point, and
truncated-moment counterexamples formalized in Lean.
\end{proof}
...
\begin{proof}[Proof idea]
Strong max-quasiconcavity makes the integrated entropy constant on the relative
interior of every fixed-support simplex face.  Apply this to the exact binary
witnesses used in Lean,
\[
p=\left(\frac13,\frac23\right),
\qquad
u=\left(\frac12,\frac12\right).
\]
They have the same support, while $H_0(p)=\log2$ and
$H_\alpha(p)<\log2=H_\alpha(u)$ for every
$\alpha\in(0,+\infty]$.  The nonnegative gap therefore has zero
$\tau$-integral, forcing $\tau((0,+\infty])=0$ and hence $\tau=\delta_0$.
\end{proof}
...
\sum_{y\in\Y}P_Y(y)H_\alpha(P_{X|Y=y}).
\end{equation}
The Lean necessity proof works directly at the measure level.  Thus consider
\begin{equation}
\vec{x}\longmapsto\|\vec{x}\|_1
...
$R=r+1$, and use the exact two-block family from
Proposition~\ref{prop:first-measure} with velocity $(0,1)$.  Its localized upper
