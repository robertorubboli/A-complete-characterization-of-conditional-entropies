...
3. `sections-4-5-full-details.tex` - the detailed proof paper, organized as the proof layer above the concise note. It includes the endpoint calculations, signed-measure differentiation, common discretization, exact localized counterexamples, tropical correction, and semiring/channel lift. A standing-notation section defines the common probability, measure, R\'enyi, semiring, order, and convergence language; each proof then defines its local thresholds, coefficients, scales, witness vectors, and asymptotic quantities before use. Expectation, variance, covariance, escort weights, total variation, and asymptotic notation are also defined explicitly.
