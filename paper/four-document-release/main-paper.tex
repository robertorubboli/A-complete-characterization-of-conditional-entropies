...
The remainder of this manuscript is structured as follows. In Section~\ref{sec: formal definitions}, we introduce the formal definitions adopted in the manuscript. In Subsection~\ref{sec: set of entropies}, we derive the general form of entropies as convex combinations of R\'enyi entropies. In Section~\ref{sec: semiring of conditional entropies}, we define the semiring of conditional majorization and determine the general form of the extremal conditional entropies $H_{t,\tau}$. The exact parameter classification used subsequently is summarized in a concise companion note~\cite{rubboli2026parametersConcise}, proved in full in Ref.~\cite{rubboli2026parametersFull}, and machine-checked in the standalone Lean formalization note~\cite{rubboli2026parametersLean}. In Section~\ref{sec: large sample}, we derive large-sample and catalytic results that form the basis for our applications. In Section~\ref{sec: set of conditional entropies}, we prove the general form of conditional entropies. In Section~\ref{sec: Applications}, we discuss applications to rates and the second laws of quantum thermodynamics with side information.
...
\label{sec:sufficient-parameter-placeholder}

The exact sufficient parameter conditions for the temperate homomorphisms,
the two tropical endpoints, and the derivations are stated with their main
proof ideas in Ref.~\cite{rubboli2026parametersConcise} and proved in full
analytic detail in Ref.~\cite{rubboli2026parametersFull}.  The latter includes
the common-discretization argument for arbitrary measures, endpoint-aware
differentiation under the parameter integral, and the lift from the one-column
curvature statements to the original semiring and channel preorder.  A
statement-by-statement machine-checked correspondence is given in
Ref.~\cite{rubboli2026parametersLean}.  This section is intentionally retained
as a placeholder so that the numbering of the present paper remains stable.

\section{Necessary conditions for the parameters}
\label{sec:necessary-parameter-placeholder}

The matching necessity results and their counterexamples are given in
Ref.~\cite{rubboli2026parametersConcise}; complete proofs appear in
Ref.~\cite{rubboli2026parametersFull}.  Those proofs use the same two-block,
three-block, Shannon, and tropical witnesses as the Lean development in
Ref.~\cite{rubboli2026parametersLean}.  They establish the parameter ranges
without assuming in advance that the singular weighted integral at
$\alpha=1$ is finite: one first proves truncated inequalities and derives the
needed one-sided finiteness only in the exceptional branch.  The resulting
admissible sets are the ones recorded in Definitions~\ref{def: D}
and~\ref{def: D_infty}.  This section is likewise a numbered placeholder.
...
As we prove later in Theorem~\ref{th:barycenter conditional entropies}, the above quantities encompass all extremal conditional entropies for appropriate values of $t$ and $\tau$. Not every parameter choice is admissible: for some choices the resulting quantity violates the axioms of Definition~\ref{def: conditional entropy}, in particular monotonicity under conditionally mixing channels. The exact admissible regimes are stated concisely in Ref.~\cite{rubboli2026parametersConcise}, proved in full in Ref.~\cite{rubboli2026parametersFull}, and formally verified in Ref.~\cite{rubboli2026parametersLean}. Consequently, we introduce the following sets of parameters and measures.
...
\end{equation}
 ...
 In the following, we show that the required convexity properties depend on the output space of the homomorphism. As used in the parameter-classification arguments of Refs.~\cite{rubboli2026parametersConcise,rubboli2026parametersFull}, the key monotonicity is monotonicity under maps acting on the conditioning subsystem of a joint distribution, which form the key subclass of conditionally mixing channels. In matrix form, for a positive joint distribution $P_{XY}$, such an action is represented as $P_{XY}\rightarrow P_{XY}D$, where $D$ is row stochastic.
