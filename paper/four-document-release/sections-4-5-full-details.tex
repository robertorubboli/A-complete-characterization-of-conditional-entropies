...
Throughout the paper, $\operatorname{supp}(\tau)$ denotes the closed support of
a positive measure $\tau$, and $\delta_a$ denotes the Dirac probability measure
at $a$.  If $T$ is Borel measurable, its push-forward is defined by
\begin{equation}
(T_\#\tau)(B):=\tau\bigl(T^{-1}(B)\bigr)
\label{eq:pushforward-definition}
\end{equation}
...
\end{equation}
For a finite signed measure $\mu$, the notation $|\mu|$ means its
total-variation measure, and
$\operatorname{supp}(\mu):=\operatorname{supp}(|\mu|)$.  A point $r$ is called
$|\mu|$-null when $|\mu|(\{r\})=0$.  These conventions are used whenever a
localization threshold is chosen below.
...
integer, $\mathbf1_E$ denotes the indicator of $E$.  As usual,
$\lceil s\rceil$ is the least integer not smaller than $s$.
...
Fix a $|\mu|$-null point $c>1$, put $p=q:=1/2$ and $R:=c+2$, and define
\begin{equation}
 n_{0,d}:=\lceil d^R\rceil,
 \qquad
...
change the argument.  We use only the implication
\begin{equation}
...
\end{equation}
Stationarity must hold exactly at the finite dimension where quasi-convexity is
...
velocity by a dimension-dependent multiple of a transverse velocity.
...
There are three counterexamples, exactly as in Lean.  If the signed Shannon
atom $m<0$ and upper coefficient $A_c>0$ are both present, use
$(h,v)=(-A_c/m,1)$.  The limiting derivatives are $0$ and $-A_c<0$; the
transverse direction $e=(0,1)$ has limiting first derivative $A_c\neq0$.
If two upper portions have coefficients $A_1,A_2>0$, use
\begin{equation}
...
Finally suppose that the unique upper point $\alpha_*$ exists.  For
$a<1<b<\alpha_*$, use $B_a\leq0$ and $A_b>0$ from
\eqref{eq:negative-truncated-coefficients}.  The tropical coefficients are
\begin{equation}
 B_a,
 \qquad
 C:=-A_b-B_a,
...
moment; after writing $\mu=-k\tau$ with $k>0$, it satisfies
\begin{equation}
 \int_{[0,+\infty]\setminus\{1\}}
 \frac{\alpha}{1-\alpha}\,\mathrm d\tau(\alpha)
 \leq0.
...
\end{equation}
If $p$ and $q$ have the same support, each can be expressed as a sufficiently
small mixture of the other with a third point on the same face.  Applying
...
$\rho=(\rho_i)_{i=1}^N$ is a finite probability vector and $f=(f_i)$ and
...
$a_s=O(b_s)$ means that $|a_s|\leq C|b_s|$ near the indicated limit, whereas
...
\end{align}
Primes and superscripts $(r)$ in this subsection denote derivatives in
$\lambda$, while $\partial_\alpha$ denotes differentiation in the R\'enyi
parameter.
...
$\ell_{\alpha,d}$ means $\ell_\alpha$ for the $d$-dimensional block path, and
unsubscripted $\mathbb E$, $\operatorname{Var}$, and $\operatorname{Cov}$ in
this subsection use the weights $\Pi_d^{(\alpha)}$.
...
\begin{align}
\mathbb E_{p(\lambda)}[f]
&:=\sum_i p_i(\lambda)f_i,
\label{eq:app-Shannon-expectation-definition}\\
\operatorname{Var}_{p(\lambda)}(f)
&:=\mathbb E_{p(\lambda)}[f^2]
-\bigl(\mathbb E_{p(\lambda)}[f]\bigr)^2,
\label{eq:app-Shannon-variance-definition}\\
\operatorname{Cov}_{p(\lambda)}(f,g)
&:=\mathbb E_{p(\lambda)}[fg]
-\mathbb E_{p(\lambda)}[f]\mathbb E_{p(\lambda)}[g].
\label{eq:app-Shannon-covariance-definition}
\end{align}
We abbreviate these subscripts to $p$ when $\lambda$ is clear, and write
...
using the block-velocity vector $z$ at dimension $d$; its primes are
$\lambda$-derivatives at zero.  The dot product is
