...
\begin{abstract}
This document is a fully dependency-closed specification for a Lean formalization
of Sections~4 and~5 of the main manuscript.  It proves the exact parameter
...
No result from another section of the manuscript may be imported.  In
...
lemma and fair-bit corollary.  This is the
normalization stated in the introduction of the main manuscript.
An isolated later occurrence of the normalization value $1$ in the attached
main file is incompatible with its displayed natural-log formula and is not
used here.  If one instead defines bit-valued entropies
$\overline H_\alpha=H_\alpha/\log2$ while retaining the natural exponential
$\exp(t\overline H)$, then the present parameter $t$ is replaced by
$t/\log2$ and the bound $M(\tau)\leq1/t$ becomes
$M(\tau)\leq\log2/t$.  If one uses $2^{t\overline H}$, the bounds in this
document remain literally unchanged.  A Lean development must select one of
these conventions globally; this blueprint uses the natural-log normalization
declared in the fair-bit corollary below.
...
apply \cref{lem:embedding-lift}.  If the formal main-body axiom is instead
normalized to $1$, divide every final functional by the positive constant
$\log2$; the order, additivity, and parameter classification are unchanged.
\end{proof}
